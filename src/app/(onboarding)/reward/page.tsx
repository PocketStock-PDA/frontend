...
/**
 * 받을 금액 — 통화별 예산(국내 1,000원 / 해외 $1)을 "금액 (약 N주)"로 표기.
 * 현재가로 나눠 수량 추정. 가격이 없으면(시세 미합성) 금액만 표기.
 */
...
export default function RewardPage() {
  ...
  // 후보별 현재가(받을 양 추정용). 국내는 KRW, 해외는 USD 현재가.
  ...
  return (
    <div className="flex min-h-screen flex-col pb-44">
      <AppHeader variant="sub" showMenu={false} title="포켓스톡 웰컴 리워드" />
      <div className="flex-1">
        <h1 className="text-2xl font-bold leading-snug text-foreground">
          어떤 주식을
          <br />
          <span className="text-primary">받아볼까요?</span>
        </h1>

        {/* 선물 배너 — 제목과 한 묶음(제안)으로 붙임 */}
        <div className="mt-4 flex items-center gap-3 rounded-2xl bg-brand-surface p-4">
        ...
        {/* 시장별 그룹 카드 */}
        {candidates.length === 0 ? (
          <EmptyState className="mt-8" title="추천 종목이 없어요" />
        ) : (
      ...
      <div className="fixed inset-x-0 bottom-0 mx-auto max-w-[430px] border-t bg-background/95 px-5 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-4 backdrop-blur">
        <div className="mb-3 rounded-xl bg-brand-surface px-4 py-3">
          <p className="text-xs font-medium text-muted-foreground">선택한 주식</p>
          {/* 선택 상태(Avatar 행) 기준으로 높이 고정 — 선택 시 카드가 커지지 않게 */}
          <div className="mt-2 h-8">
            {/* 선택이 바뀔 때마다 새 요약이 부드럽게 차오름 — 선택 반영 확인 피드백 */}
            <motion.div
              key={selected ? selected.stockCode : "none"}
              ...
              {selected ? (
                <>
                  <Avatar className="size-8 shrink-0">
                    {selected.logoUrl && <AvatarImage src={selected.logoUrl} alt="" />}
                    <AvatarFallback className="text-xs">
                      {selected.stockName.trim().charAt(0).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  <p className="min-w-0 flex-1 truncate text-sm font-bold text-foreground">
                    {selected.stockName}
                  </p>
                  <p className="shrink-0 font-numeric text-sm font-bold text-primary">
      ...
      {granted && (
        <GrantedOverlay
          reward={granted}
          logoUrl={
            candidates.find((c) => c.stockCode === granted.stockCode)?.logoUrl ??
            null
          }
          reduce={!!reduce}
  ...
  return (
    ...
    >
      <div className="relative mb-6 size-24">
        <Avatar className="absolute inset-0 m-auto size-[72px]">
